	...
	public function __call($method, $args){
		if(in_array($method, [
			...
			if($this->cache_key != $this->primary_key 
				&& strpos($method, 'by_primary_key') === false
			){
				$key	= $this->get_cache_key($key);
			}else{
				$key	= $this->get_cache_key($key, true);
			}

			if(in_array($method, ['cache_get', 'cache_get_by_primary_key'])){
				return wp_cache_get($key, $this->cache_group);
			}elseif(in_array($method, ['cache_delete', 'cache_delete_by_primary_key'])){
				return wp_cache_delete($key, $this->cache_group);
			}else{
				$data		= $args[1];
				$cache_time	= !empty($args[2]) ? (int)$args[2] : $this->cache_time;

				if($method == 'cache_add'){
					return wp_cache_add($key, $data, $this->cache_group, $cache_time);
				}else{
					return wp_cache_set($key, $data, $this->cache_group, $cache_time);
				}
			}
		}elseif(in_array($method, [
			'get_meta',
